...
/obj/item/reagent_containers/food/drinks/soda_cans/inf/Initialize(mapload)
	. = ..()
	for(var/i in 1 to 5)
		reagents.addreagent(get_random_reagent_id(), 10)

/datum/round_event/infcola/start()
	for(var/i in 1 to rand(5, 10))
		var/obj/item/reagent_containers/food/drinks/soda_cans/inf/newCan = new(src)
		var/turf/warpoint = find_safe_turf(extended_safety_checks = TRUE, dense_atoms = FALSE)
		newCan.forceMove(warpoint)
		do_smoke(location=warpoint)
